...
class TestHealthEndpoint:
    async def test_health_returns_200_when_db_is_up(self, client):
        ...
        with patch(
            "app.core.health.check_database_health",
            new_callable=AsyncMock,
            return_value=True,
        ), patch(
            "app.core.health.check_tmdb_health",
            new_callable=AsyncMock,
            return_value=True,
        ), patch(
            "app.core.health.get_memory_usage",
            return_value={"healthy": True, "used_percent": 50},
        ):
    ...
    async def test_health_response_structure(self, client):
        ...
        with patch(
            ...
            "app.core.health.get_memory_usage",
            return_value={"healthy": True, "used_percent": 40},
        ):
            response = await client.get("/health")
    ...
    async def test_health_checks_contain_database_key(self, client):
        with patch(
            ...
            "app.core.health.get_memory_usage", return_value={"healthy": True}
        ):
            response = await client.get("/health")
    ...
    async def test_healthy_when_all_checks_pass(self, client):
        with patch(
            ...
            "app.core.health.get_memory_usage",
            return_value={"healthy": True, "used_percent": 30},
        ):
            response = await client.get("/health")
    ...
    async def test_unhealthy_when_memory_is_critical(self, client):
        with patch(
            ...
            "app.core.health.get_memory_usage",
            return_value={"healthy": False, "used_percent": 95},
        ):
            response = await client.get("/health")
...
class TestCheckDatabaseHealth:
    async def test_returns_true_when_ping_succeeds(self):
        ...
        with patch("app.db.connection.client", mock_client):
    ...
    async def test_returns_false_when_client_is_none(self):
        with patch("app.db.connection.client", None):
            from app.core.health import check_database_health
...
class TestGetMemoryUsage:
    ...
    def test_healthy_is_false_when_psutil_fails(self):
        with patch(
            "app.core.health.psutil.virtual_memory", side_effect=Exception("No psutil")
        ):
            from app.core.health import get_memory_usage
