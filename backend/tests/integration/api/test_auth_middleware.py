...
class TestAuthenticationMiddleware:
    ...
    async def test_get_current_user_info_success(
        self, client: AsyncClient, auth_token, test_user
    ):
    ...
    async def test_get_current_user_info_expired_token(
        ...
        headers = {
            "Authorization": "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0ZXN0dXNlciIsImV4cCI6MTYwMDAwMDAwMH0.invalid"
        }
        response = await client.get("/api/auth/me", headers=headers)
    ...
    async def test_logout_success(self, client: AsyncClient, auth_token, test_user):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            f"User {test_user['username']} logged out successfully"
