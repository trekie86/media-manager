...
async def create_storage(storage_data_input: dict, db=Depends(get_database)):
    ...
    try:
        ...
        response_data = {
            ...
            "parent_id": (
                str(created_storage["parent_id"])
                if created_storage.get("parent_id")
                else None
            ),
...
async def get_storage(storage_id: str, db=Depends(get_database)):
    ...
    try:
        ...
        response_data = {
            ...
            "type": storage["type"],
            "parent_id": (
                str(storage["parent_id"]) if storage.get("parent_id") else None
            ),
    ...
    try:
        ...
        if storage.get("path"):
            ...
            for ancestor in ancestors:
                ancestor_dict = {
                    ...
                    "type": ancestor["type"],
                    "parent_id": (
                        str(ancestor["parent_id"])
                        if ancestor.get("parent_id")
                        else None
                    ),
        ...
        for descendant in descendants:
            descendant_dict = {
                ...
                "parent_id": (
                    str(descendant["parent_id"])
                    if descendant.get("parent_id")
                    else None
                ),
...
async def update_storage(
    ...
    try:
        ...
        response_data = {
            ...
            "parent_id": (
                str(updated_storage["parent_id"])
                if updated_storage.get("parent_id")
                else None
            ),
